...
function populateArtists(data) {
    ...
    data.forEach(element => {
        result += `<tr class='table-row' data-toggle='collapse' data-target='.row1-details' aria-expanded='false'>
        <td>
          <img id='arrowOpen' height='30px' src='img/arrowclosed.png' alt='Closed arrow'>
          <img id='arrowClosed' class='hide' width='30px' height='30px' src='img/arrowopen.png' alt='Open arrow'>
        </td>
        <td>` + element.name + `</td>
